...
export default function Component1() {
    ...
    const selectfun = (event) => {
        ...
        var z = Number(event.target.value);
        var x = z * 60;

        total = utchour * 60 + utcminute
        remaining = total + x;
        var newhr = parseInt(remaining / 60);
        var newminute = remaining % 60
        setHour(newhr)
        setMinute(newminute)
        setSecond(utcsecond)
if(second!==''){
...
interval = setInterval(() => {
    ...
    return (
        <div id="component1">
            ...
            <h2>Select the time zone and get the time</h2>
            ...
            <p style={{ color: "white", fontSize: "5vw", marginTop:"10%" }}>{hour} : {minute} : {second}</p>
        </div>
    )
}
